...
from zuse.agent.inference import InferenceClient, InferenceError, InferenceRequest
from zuse.agent.prompt import SYNTHESIS_SCHEMA, build_synthesis_prompt
from zuse.agent.runspec import RunSpec
from zuse.agent.scaffold import java_signature as scaffold_java_signature
from zuse.agent.segment import Paragraph, segment
from zuse.agent.validate import (
...
if __name__ == "__main__":
    ...
    cobol_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("zuse/examples/input/interest.cob")
    scaffold_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("zuse/examples/generated/Scaffold.java")
    out_dir = Path(sys.argv[3]) if len(sys.argv) > 3 else Path("zuse/examples/generated/synthesis")

    paragraphs = {p.identifier: p for p in segment(cobol_path.read_text(encoding="utf-8"))}
    target = paragraphs["PROCESS-RECORD"]

    client = InferenceClient(cache_dir=Path("zuse/examples/generated/model_cache"))
