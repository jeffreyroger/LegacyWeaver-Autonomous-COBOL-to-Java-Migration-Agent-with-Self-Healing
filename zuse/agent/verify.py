...
Compares against `zuse/examples/reference/interest.out` rather than
...
nothing extra here. `zuse verify`'s live-oracle path remains the
...
from zuse.classification import Classification, classify
...
GOLDEN_OUTPUT = Path("zuse/examples/reference/interest.out")
INPUT_DATA = Path("zuse/examples/reference/accounts.dat")
