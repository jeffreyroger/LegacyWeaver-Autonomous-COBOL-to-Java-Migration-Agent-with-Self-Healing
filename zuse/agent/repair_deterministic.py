...
in `zuse.layout` -- no model call is needed to fix them, only to
recognize them (classification, already deterministic per zuse.classification).
...
def patch_scale(body: str, target_scale: int) -> Patch:
    ...
    PIC clause states it -- zuse.layout's decimal_scale for the field).
