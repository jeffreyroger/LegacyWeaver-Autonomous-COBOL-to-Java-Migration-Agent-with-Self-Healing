"""Surface tests for `zuse migrate` (SRS SS3.9.1). These never invoke the
...
from zuse.cli import build_migrate_spec, build_parser
...
def test_migrate_writes_an_fr_8_1_run_dir(tmp_path, monkeypatch):
    ...
    exactly what `zuse report` reads."""
    import zuse.cli as cli_mod
...
def test_stream_event_emits_colour_coded_status(capsys):
    ...
    """
    from zuse.cli import _stream_event
...
def test_verify_accepts_the_srs_flag_form():
    """SS3.9.1: zuse verify --cobol <src> --java <src> --data <file>."""
    args = build_parser().parse_args(
