"""Single `verify` command (Step F1/F2; FR-15).
    ...
    zuse verify <cobol_source> <java_candidate> <input_data> [--report OUT]
...
from zuse.agent.orchestrator import RUNS_ROOT, Orchestrator
from zuse.agent.program_profiles import program_profile as _program_profile
from zuse.agent.runspec import (
    DEFAULT_MAX_REPAIRS,
    DEFAULT_MODEL,
    DEFAULT_SEED,
    RunSpec,
)
from zuse.classification import classify, summarize
from zuse.comparison import compare_lines, normalize_line_endings
from zuse.execution import run_candidate, run_oracle
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="zuse", description="Zuse differential verification harness")
    ...
    # SRS 3.9.1: zuse migrate <program.cbl> [--copybook DIR] [--data FILE]
...
def run_report(args: argparse.Namespace) -> int:
    """`zuse report <run_dir>` (BACKEND_PLAN.md §4.4 DC-5 target).
    ...
    GET /runs/{id} serves -- both call zuse.agent.metrics.compute_metrics.
    """
...
def _render_migrate_summary(run_dir: Path, results: dict) -> None:
    ...
    console.print(f"[cyan]Metrics:[/cyan] zuse report {run_dir}")


def run_migrate(args: argparse.Namespace) -> int:
    """`zuse migrate` (SRS 3.9.1) -- drive the orchestrator from the terminal.

    Constructs the same Orchestrator backend/runs.py constructs and writes the
    same files into the run directory, so `zuse report <run_dir>` and the
    ...
    # Every value here actually influences the run -- see zuse/agent/runspec.py.
