...
from zuse.classification import DefectClass, classify, summarize
from zuse.comparison import compare_lines, normalize_line_endings

REPO_ROOT = Path(__file__).resolve().parent.parent
GOLDEN = REPO_ROOT / "examples" / "reference" / "interest.out"
ACCOUNTS = REPO_ROOT / "examples" / "reference" / "accounts.dat"
...
def test_ac8_single_field_alteration_localises_correctly():
    ...
    from zuse.layout import REPORT_LAYOUT
...
def test_full_pipeline_against_real_fixture(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "zuse.cli", "verify",
         "examples/input/interest.cob", "examples/baseline/Baseline.java", "examples/reference/accounts.dat",
    ...
    # zuse/report.py's DIVERGENCE_CAP) -- reclassifying only that sample
    ...
    # zuse.cli.run_verify itself does for its terminal/JSON classification
...
def _classify_all_divergences() -> list:
    ...
    zuse.cli.run_verify's own in-memory (uncapped) computation exactly --
    see AC-10's comment above for why report.json's capped list isn't enough.
    """
    from zuse.cli import _compile_candidate
    from zuse.execution import run_candidate
    from zuse.layout import REPORT_LAYOUT, TOTALS_LAYOUT

    main_class, classpath = _compile_candidate(REPO_ROOT / "examples" / "baseline" / "Baseline.java")
