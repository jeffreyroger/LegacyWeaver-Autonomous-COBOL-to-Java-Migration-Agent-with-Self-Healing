...
from zuse.agent.assemble import assemble
from zuse.agent.attribution import verify_unit
from zuse.agent.escalation import DiagnosticRecord, build_diagnostic_record
from zuse.agent.inference import GROQ_HOST, InferenceClient
from zuse.agent.memory import FailureMemory, MemoryCase, embed
from zuse.agent.memory_repair import try_memory_repair
from zuse.agent.repair_loop import repair_unit
from zuse.agent.runspec import RunSpec
from zuse.agent.scaffold import field_scale as scaffold_field_scale
from zuse.agent.scaffold import generate as generate_scaffold
from zuse.agent.segment import Paragraph, segment
from zuse.agent.signature import build_signature
from zuse.agent.synthesize import SynthesisResult, synthesize_paragraph
from zuse.agent.validate import SynthesizedBody
...
TRACE_PATH = Path("zuse/examples/generated/trace.jsonl")  # FR-8.1
RUNS_ROOT = Path("zuse/examples/generated/runs")  # FR-8.1: runs/<run_id>/trace.jsonl
...
class Orchestrator:
    ...
    def __post_init__(self) -> None:
        ...
        # `zuse migrate`/the backend work for any program, not only
        ...
        # GitHub Action sets ZUSE_INFERENCE_PROVIDER=groq because runners
        # have no local Ollama daemon. Local/CLI/backend/frontend runs never
        # set this and stay on the offline "ollama" default.
        provider = os.environ.get("ZUSE_INFERENCE_PROVIDER", "ollama")
        client_kwargs: dict = {
            "cache_dir": Path("zuse/examples/generated/model_cache"),
    ...
    def _process_unit(self, unit: Paragraph) -> UnitResult:
        ...
        t0 = time.monotonic()
        result = verify_unit(unit.identifier, body, Path(f"zuse/examples/generated/orchestrator/{unit.identifier}/synthesis"),
                              spec=self.spec)
        ...
        if result.compiled:
            ...
            mem_result, case_id = try_memory_repair(
                ...
                failing_div.field_name, Path(f"zuse/examples/generated/orchestrator/{unit.identifier}/memory"),
                spec=self.spec,
            )
        ...
        outcome = repair_unit(unit.identifier, unit, body, result, self.client,
                               Path(f"zuse/examples/generated/orchestrator/{unit.identifier}/repair"), spec=self.spec)
    ...
    def _persist_state(self) -> None:
        ...
        # run is still executing (see zuse/atomic_json.py).
