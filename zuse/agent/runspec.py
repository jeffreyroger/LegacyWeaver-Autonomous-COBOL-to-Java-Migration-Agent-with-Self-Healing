...
One field per SRS SS3.9.1 `zuse migrate` flag. Before this existed,
...
from zuse.agent.scaffold import INTEREST_SPEC, ScaffoldSpec
...
DEFAULT_COBOL_SOURCE = Path("zuse/examples/input/interest.cob")
DEFAULT_INPUT_DATA = Path("zuse/examples/input/data/accounts.dat")
DEFAULT_GOLDEN_OUTPUT = Path("zuse/examples/reference/interest.out")
DEFAULT_SCAFFOLD_PATH = Path("zuse/examples/generated/Scaffold.java")
DEFAULT_MEMORY_STORE = Path("zuse/examples/generated/failure_memory.json")
DEFAULT_REFERENCE_BODY_PATH = Path("zuse/examples/reference/process_record.body.java")
...
class RunSpec:
    ...
    # SS3.9.1's `zuse migrate` flag surface, so zuse/cli.py never sets
