...
from zuse.agent.orchestrator import UnitResult
...
def _create_payload(**overrides):
    payload = dict(
        cobol_source="zuse/examples/input/interest.cob", data_file="zuse/examples/input/data/accounts.dat",
...
def test_get_run_includes_diagnostic_for_escalated_unit(client, monkeypatch):
    ...
    from zuse.agent.escalation import DiagnosticRecord
...
def test_escalation_decision_verifies_against_the_run_own_spec(client, monkeypatch):
    ...
    from zuse.agent.attribution import AttributionResult
    from zuse.agent.escalation import DiagnosticRecord
    ...
    resp = client.post("/runs", json=_create_payload(
        cobol_source="zuse/examples/input/feecalc.cob", data_file="zuse/examples/input/data/fees.dat",
    ))
    run_id = resp.json()["run_id"]
    _wait_terminal(client, run_id)
...
def test_cli_report_and_api_metrics_are_byte_identical(tmp_path, capsys):
    ...
    from zuse.agent.metrics import compute_metrics
    from zuse.cli import build_parser, run_report
