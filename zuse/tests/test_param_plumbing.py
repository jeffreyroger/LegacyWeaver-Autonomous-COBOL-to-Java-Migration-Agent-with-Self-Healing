...
had no effect (DC-5). SRS SS3.9.1 requires `zuse migrate` to expose seven
...
def test_default_paths_match_repo_fixtures():
    spec = RunSpec.default()
    assert spec.input_data == Path("zuse/examples/input/data/accounts.dat")
    assert spec.golden_output == Path("zuse/examples/reference/interest.out")
    assert spec.scaffold_path == Path("zuse/examples/generated/Scaffold.java")
...
def test_verify_unit_reads_the_scaffold_it_was_given(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zuse.agent.attribution.assemble", fake_assemble)
...
def test_backend_build_run_spec_threads_every_request_field(tmp_path):
    ...
    assert spec.copybook_dir == Path("zuse/examples/input/copybooks")
...
def test_verify_candidate_uses_injected_input_data(tmp_path, monkeypatch):
    from zuse.agent import verify as verify_mod
